...
var requireLogin = function() {
	if (!Meteor.user()) {
		...
		var free_account_expires = moment(Meteor.user().group.group_created_on).add(Controls.findOne().freeTrial, 'd').utc().valueOf();
...
var startUp = function() {
	if (Session.get('logout_time')) {
		Meteor.call('updateRemember', Session.get('remember_me'), Session.get('logout_time'));
	}
...
var checkTimeout = function() {
	var initialCheckTimeout = setInterval(function() {
		if (Meteor.user()) {
			...
			var current_date = moment().utc().valueOf();
			var logout_time = moment(Meteor.user().profile.logout_time).utc().valueOf();
			if (Meteor.user().profile.remember_me === false && logout_time <= current_date) {
				Meteor.logout();
			}
			if (Meteor.user().role.inactive) {
				Meteor.logout();
			}
...
Router.onBeforeAction(requireLogin, {except: ['home', 'pricing', 'support', 'supportSection', 'contact', 'facebook', 'twitter', 'linkedin', 'login', 'register', 'emailNotVerified', 'emailVerified', 'reset', 'resetPassword', 'passwordLinkSent', 'exportLoading']});
Router.onAfterAction(startUp, {except: ['exportLoading']})
Router.onAfterAction(checkTimeout, {except: ['home', 'pricing', 'support', 'supportSection', 'contact', 'facebook', 'twitter', 'linkedin', 'login', 'register', 'emailNotVerified', 'emailVerified', 'reset', 'resetPassword', 'passwordLinkSent', 'exportLoading']})
Router.onAfterAction(searchClear, {except: ['contactsSearch', 'contactInfo']})
...
Router.route('exportLoading', {
	...
	action: function () {
		...
		var groupId = Meteor.users.findOne({_id: userId}).group.group_id;
...
Router.route('contactNew', {
	...
	waitOn: function() {
		var userId = Meteor.userId()
		return Meteor.subscribe('counts', 'all_contacts_tag', userId)
	},
	onAfterAction: function () {
		$('.js_sheet_action').slideUp('fast');
