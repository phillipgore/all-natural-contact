Template.officeSupportList.onRendered(function() {
  this.autorun(function() {
    ...
    var checkCurrentSection = setInterval(function() {
      if (Support.find().count() === SupportCount.findOne().support_count) {
        ...
        $('#' + Session.get('currentSection')).find('.js_support_list_item').addClass('active');
      }
    }, 300)
  });

  $('.js_tool').removeClass('active')
  $('.js_tool_office_support').addClass('active');
});

Template.officeSupportList.helpers({
  sections: function() {
...
Template.officeSupportList.events({
