...
Template.conversationUpdate.helpers({
	contact: function() {
		return Contacts.findOne(Session.get('currentContact'));
	},
});

Template.conversationUpdate.events({
	...
	'submit form': function(e) {
		...
		Meteor.call('conversationUpdate', Session.get('currentContact'), conversation_id, conversationProperties, function(error, result) {
			if (error) {
				return alert(error.reason);
			} else {
				Session.set({conScrollDir: 'middle', conPivotId: conversation_id, conPivotDate: conversationProperties.conversation_date});
